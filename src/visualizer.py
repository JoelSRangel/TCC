...
class EmbeddingVisualizer:
    def __init__(self, embeddings_list):
        ...
        import numpy as np
        self.vector_matrix = np.array([item['embedding'] for item in embeddings_list])

    def generate_scatter_plot(self, target_terms=None, output_filename="clusters_transe.png"):
        print("[INFO] Reduzindo dimensões espaço-temporais via PCA (64D -> 2D)...")
        ...
        plt.title("Projeção do Espaço Latente TransE (Lote Completo SemClinBr - PyKeen)", fontsize=14, pad=15)
        ...
        plt.close()
        print(f"[SUCESSO] Gráfico vetorial salvo em: {output_path}")