...
class EmbeddingVisualizer:
    ...
    def generate_scatter_plot(self, target_terms=None, output_filename="clusters_rotate.png"):
        print("[INFO] Reduzindo dimensões via PCA (64D RotatE -> 2D)...")
            ...
            for j, text in enumerate(self.texts):
                ...
                if cleaned_text in target_lower:
                    plt.annotate(text, (vectors_2d[j, 0], vectors_2d[j, 1]), 
                                 ...
                                 bbox=dict(boxstyle="round,pad=0.3", fc="lightblue", alpha=0.8)) # Mudamos para azul claro no RotatE
                    plt.scatter(vectors_2d[j, 0], vectors_2d[j, 1], color='red', s=150, edgecolors='black', zorder=5)

        plt.title("Projeção do Espaço Latente RotatE (Lote Completo SemClinBr)", fontsize=14, pad=15)
