...
class EmbeddingVisualizer:
    ...
    def generate_scatter_plot(self, target_terms=None, output_filename="clusters_globais_semclinbr.png"):
        print("[INFO] Aplicando redução de dimensionalidade via PCA (64D -> 2D)...")
        pca = PCA(n_components=2, random_state=42)
        vectors_2d = pca.fit_transform(self.vector_matrix)

        # Aumentamos a largura da figura para acomodar a legenda gigante de 998 arquivos
        plt.figure(figsize=(15, 9))
        
        unique_tags = list(set(self.tags))
        colors = plt.cm.get_cmap('tab20', len(unique_tags)) # Usamos tab20 que tem mais variedade de cores
        
        for i, tag in enumerate(unique_tags):
            indices = [j for j, t in enumerate(self.tags) if t == tag]
            plt.scatter(vectors_2d[indices, 0], vectors_2d[indices, 1], 
                        alpha=0.4, label=tag, s=30, color=colors(i))

        if target_terms:
            target_lower = [t.lower() for t in target_terms]
            for j, text in enumerate(self.texts):
                if text.lower() in target_lower:
        ...
        plt.title("Projeção Bidimensional Global do Espaço Latente (Lote Completo SemClinBr)", fontsize=14, pad=15)
        plt.xlabel("Componente Principal 1", fontsize=11)
        plt.ylabel("Componente Principal 2", fontsize=11)
        
        # Diminuímos o tamanho da fonte da legenda (prop={'size': 7}) para caber todas na tela
        plt.legend(title="Categorias Semânticas", bbox_to_anchor=(1.02, 1), loc='upper left', prop={'size': 7.5})
        plt.grid(True, linestyle='--', alpha=0.3)
        
        # Ajustamos os subplots manualmente deixando uma margem generosa na direita (right=0.75) para a legenda
        ...
        plt.close()
        print(f"[SUCESSO] Novo gráfico global gerado e salvo em: {output_path}")